...
const BookTable: React.FC<TableProps> = ({ title, columns, data, actions }) => {
    return (
        ...
        >
            {title && <h2 className="border-b bg-gray-50 p-4 text-lg font-semibold text-gray-800">{title}</h2>}
                    ...
                    {data.length > 0 ? (
                        data.map((item, index) => (
                            <motion.tr
                                ...
                                whileHover={{ scale: 1.02, backgroundColor: '#f8fafc', boxShadow: '0px 2px 10px rgba(0,0,0,0.05)' }}
                                className="transition-all"
                            >
                                {columns.map((col) => (
                                    <td key={col.key} className="px-6 py-4 text-gray-800">
                                        {col.key === 'return_status' ? (
                                            <span
                                                className={`rounded-full px-3 py-1 text-sm font-medium ${
                                                    item[col.key] === 'Returned' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                                }`}
                                            >
                                                {item[col.key]}
                                            </span>
