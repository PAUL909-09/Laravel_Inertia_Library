...
import BookTable from '../components/Table/Table'; // Import the BookTable component
...
const Dashboard: React.FC<DashboardProps> = ({ borrowings, totalUsers, totalBooks, totalBorrowedBooks }) => {
    // Transform the borrowings data to match the flat structure expected by BookTable
    const tableData = borrowings.map((borrowing) => ({
        id: borrowing.id,
        book: borrowing.book.title, // Flatten book.title to book
        user: borrowing.user.name, // Flatten user.name to user
        borrowed_date: borrowing.borrowed_date,
        return_status: borrowing.return_date ? 'Returned' : 'Not Returned', // Compute return status
    }));

    // Define the columns for the BookTable
    const columns = [
        { key: 'book', label: 'Book' },
        { key: 'user', label: 'User' },
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="mx-auto max-w-6xl space-y-6">
            ...
            {/* Borrowed Books Table using BookTable component */}
            <BookTable title="Borrowed Books" columns={columns} data={tableData} />
        </motion.div>
    );
};
